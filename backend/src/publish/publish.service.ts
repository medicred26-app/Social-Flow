...
export class PublishService {
  ...
  private assertNoMock(token: string) {
  ...
  private async refreshIfNeeded(supabase: SupabaseClient, account: AccountRecord) {
    if (!account.token_expires_at) return this.crypto.decryptSecret(account.access_token_encrypted);
    const expires = new Date(account.token_expires_at).getTime();
    if (expires - Date.now() > 60_000) {
      return this.crypto.decryptSecret(account.access_token_encrypted);
    }
    if (!account.refresh_token_encrypted) {
      return this.crypto.decryptSecret(account.access_token_encrypted);
    }

    const refreshToken = this.crypto.decryptSecret(account.refresh_token_encrypted);
    let accessToken = '';
    let newRefresh = account.refresh_token_encrypted;
    let expiresAt = account.token_expires_at;

    if (account.platform === 'x') {
      const { clientId, clientSecret } = this.platformEnv.getPlatformCredentials('x');
      ...
      if (data.refresh_token) newRefresh = this.crypto.encryptSecret(data.refresh_token);
      if (data.expires_in) expiresAt = new Date(Date.now() + Number(data.expires_in) * 1000).toISOString();
    } else if (account.platform === 'youtube') {
      const { clientId, clientSecret } = this.platformEnv.getPlatformCredentials('youtube');
      ...
      const { clientId, clientSecret } = this.platformEnv.getPlatformCredentials('linkedin');
    ...
    this.assertNoMock(accessToken);
    await supabase
      .from('connected_accounts')
      .update({
        access_token_encrypted: this.crypto.encryptSecret(accessToken),
  ...
  private async publishFacebook(token: string, account: AccountRecord, input: PublishInput) {
  ...
  private async publishInstagram(token: string, account: AccountRecord, input: PublishInput) {
    if (!input.mediaUrl) {
    ...
    const container = await this.readJson(containerRes);
    ...
    const published = await this.readJson(publishRes);
    return published.id;
  }

  private async publishLinkedIn(token: string, account: AccountRecord, input: PublishInput) {
  ...
  private async publishX(token: string, input: PublishInput) {
  ...
  private async publishYouTube(token: string, input: PublishInput) {
    if (!input.mediaUrl) {
    ...
    const data = await this.readJson(upload);
    return data.id;
  }

  async publishToAccount(supabase: SupabaseClient, account: AccountRecord, input: PublishInput) {
    const token = await this.refreshIfNeeded(supabase, account);
    this.assertNoMock(token);

    if (account.platform === 'facebook') return this.publishFacebook(token, account, input);
    if (account.platform === 'instagram') return this.publishInstagram(token, account, input);
    if (account.platform === 'linkedin') return this.publishLinkedIn(token, account, input);
    if (account.platform === 'x') return this.publishX(token, input);
    return this.publishYouTube(token, input);
  }

  async publishPostById(supabase: SupabaseClient, postId: string, userId?: string) {
    ...
    for (const target of targets || []) {
      ...
      try {
        const platformPostId = await this.publishToAccount(supabase, account as AccountRecord, {
  ...
  async processDuePosts(supabase: SupabaseClient) {
    ...
    for (const row of due || []) {
      processed.push(await this.publishPostById(supabase, row.id));
    }
